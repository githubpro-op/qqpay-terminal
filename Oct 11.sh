...
<!-- ⚡ QQWALLET FUTURISTIC FIXED UI — V3.6 DYNAMIC QR PATH ENABLED -->
<div id="wallet-modal" class="modal">
  <div class="modal-content wallet-neon">
    ...
    <div class="wallet-balance">
      <div class="bal-card"><h4>₹ INR</h4><p id="wBalRS">₹0.00</p></div>
      <div class="bal-card"><h4>USDT (₮)</h4><p id="wBalUSDT">0.0000 ₮</p></div>
      <div class="bal-card"><h4>QQ COINS</h4><p id="wBalQQ">0.0000 QQ</p></div>
    </div>

    <!-- ACTION BUTTONS -->
    <div class="wallet-actions">
      <button class="wallet-btn dep" onclick="qqOpenForm('depositForm')">💰 Deposit</button>
      <button class="wallet-btn wit" onclick="qqOpenForm('withdrawForm')">🏧 Withdraw</button>
      <button class="wallet-btn exg" onclick="qqOpenForm('exchangeForm')">♻️ Exchange</button>
      <button class="wallet-btn trf" onclick="qqOpenForm('transferForm')">🔁 Transfer</button>
    </div>
    ...
    <div id="walletForms">
      ...
      <div id="depositForm" class="form-box hidden">
        <h3>Deposit Funds</h3>
        <input type="number" id="depositAmt" placeholder="Enter amount in ₹" min="1" class="input-dark">
    ...
    <div class="wallet-history">
      <div class="history-head">🧾 Recent Transactions</div>
...
// 🧩 Load deposit methods from backend (live)
async function loadDepositMethods(){
  try{
    const res = await fetch('/deposit/methods');
    depositMethods = await res.json();
  }catch(e){ showPopup('Failed to load deposit methods'); }
}

// 💰 Deposit method selector (QR path + save button)
function qqDepositMethod(type){
  selectedDepositMethod = type;
  const method = depositMethods.find(m=>m.type===type);
  if(!method){
    document.getElementById('depositDetails').innerHTML = '<div>No method available</div>';
    return;
  }
  let html = `<div><b>${method.label}</b><br>${method.details}</div>`;
  if(type==='upi' || type==='usdt'){
    const qrUrl = `/storage/${method.qr_path}`;
    html += `
      <div style='margin-top:10px;'>
        <img src='${qrUrl}' width='140' height='140' style='border-radius:10px;border:1px solid #444;'>
      </div>
      <div style='margin-top:8px;display:flex;gap:8px;justify-content:center;'>
        <button class='primary' onclick="copyToClipboard('${method.details}')">📋 Copy</button>
        <a href='${qrUrl}' download class='primary' style='text-decoration:none;text-align:center;'>💾 Save QR</a>
      </div>
    `;
  }
...
function copyToClipboard(text){
  navigator.clipboard.writeText(text).then(()=>showPopup('Copied!'));
}

function qqWithdrawMethod(m){
  selectedWithdrawMethod=m;
  let html='';
  if(m==='upi')html='<input class="input-dark" placeholder="Enter UPI ID">';
  if(m==='bank')html='<input class="input-dark" placeholder="Account Name"><input class="input-dark" placeholder="Account No"><input class="input-dark" placeholder="IFSC">';
  if(m==='usdt')html='<input class="input-dark" placeholder="Your TRC20 Address">';
  document.getElementById('withdrawDetails').innerHTML=html;
}

async function loadWalletData(){
  try{
    const r=await fetch('/wallet/{{ Auth::id() }}');
    const d=await r.json();
    document.getElementById('wBalRS').innerText='₹'+parseFloat(d.wallet.balance_rs).toFixed(2);
    document.getElementById('wBalUSDT').innerText=parseFloat(d.wallet.balance_usdt).toFixed(4)+' ₮';
    document.getElementById('wBalQQ').innerText=parseFloat(d.wallet.balance_qq).toFixed(4)+' QQ';
    const tb=document.getElementById('transaction-history');
    tb.innerHTML='';
    if(!d.transactions||!d.transactions.length)tb.innerHTML='<tr><td colspan="5">No Transactions</td></tr>';
    else d.transactions.forEach(t=>tb.innerHTML+=`<tr><td>${t.type}</td><td>${t.amount}</td><td>${t.method}</td><td>${t.status}</td><td>${new Date(t.created_at).toLocaleString()}</td></tr>`);
  }catch(e){showPopup('Wallet load failed');}
}

async function sendWalletReq(url,body){
  try{
    ...
    const d=await r.json();
    showPopup(d.message||'Done');
    loadWalletData();
  }catch(e){showPopup('Action failed');}
}

async function submitWalletDeposit(){
  if(!selectedDepositMethod){showPopup('Select deposit method!');return;}
  if(!depositAmt.value){showPopup('Enter amount!');return;}
  const formData=new FormData();
  formData.append('amount',depositAmt.value);
  formData.append('method',selectedDepositMethod);
  const file=document.getElementById('depositReceipt').files[0];
  if(file)formData.append('receipt',file);
  try{
    const r=await fetch('/wallet/deposit',{method:'POST',headers:{'X-CSRF-TOKEN':'{{ csrf_token() }}'},body:formData});
    const d=await r.json();
    showPopup(d.message||'Deposit done!');
    loadWalletData();
  }catch(e){showPopup('Deposit failed');}
}

async function submitWalletWithdraw(){
  if(!selectedWithdrawMethod){showPopup('Select withdraw method!');return;}
  if(!withdrawAmt.value){showPopup('Enter amount!');return;}
  await sendWalletReq('/wallet/withdraw',{amount:withdrawAmt.value,method:selectedWithdrawMethod});
}

async function submitWalletExchange(){
  await sendWalletReq('/wallet/exchange',{from:exchangeFrom.value,to:exchangeTo.value,amount:exchangeAmt.value});
}

async function submitWalletTransfer(){
  await sendWalletReq('/wallet/transfer',{to_id:transferID.value,mode:transferMode.value,amount:transferAmt.value});
}

document.addEventListener('DOMContentLoaded',()=>{loadDepositMethods();loadWalletData();});
</script>
EOF
